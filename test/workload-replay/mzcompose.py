...
def extract_docker_series(
    ...
    if not docker_stats:
        raise ValueError("docker_stats is empty")
...
def compare_table(stats_old: dict[str, Any], stats_new: dict[str, Any]) -> None:
    rows = []
    ...
    if "queries" in stats_old:
        ...
        rows.extend(
            [
                ...
                ("Query avg (ms)", old_q["avg"] * 1000, new_q["avg"] * 1000, 1.2),
                ("Query p50 (ms)", old_q["p50"] * 1000, new_q["p50"] * 1000, 1.2),
    ...
    for name, old, new, threshold in rows:
        ...
        if threshold is not None and new > old * threshold:
            regressed = True
            flag = "!!YES!!"
        else:
            flag = "no"
    ...
    if regressed:
        raise ValueError("Regression caught!")
...
def benchmark(
    ...
    compare_table(stats_old, stats_new)
...
def test(
    ...
    stop_event = threading.Event()
    stats = {
        "queries": {"total": 0, "failed": 0, "slow": 0, "timings": []},
