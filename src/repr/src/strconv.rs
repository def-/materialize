...
fn parse_timestamptz_inner(
    ...
    // Roll a `:60` second over only after applying the offset: the local
    // rollover can leave chrono's range while the UTC instant it denotes stays
    // inside it, as in `262142-12-31 23:59:60+01`.
    let utc = dt
        .checked_sub_offset(offset)
        .and_then(roll_over_leap_second)
        .ok_or_else(|| ParseError::out_of_range("timestamp with time zone", s))?;
    CheckedTimestamp::from_timestamplike(DateTime::from_naive_utc_and_offset(utc, Utc))
...
mod tests {
    ...
    /// chrono's max date parses, and adding the rollover second overflows. The
    /// leap value on the max date errors as out of range instead, unless an
    /// offset brings the UTC instant it denotes back into range.
    #[mz_ore::test]
    fn leap_second_rollover_at_max_date() {
        let err_kind = |s| parse_timestamptz(s).unwrap_err().kind;
        assert_eq!(
            parse_timestamp("262142-12-31 23:59:60").unwrap_err().kind,
            ParseErrorKind::OutOfRange,
        );
        assert_eq!(
            err_kind("262142-12-31 23:59:60+00"),
            ParseErrorKind::OutOfRange
        );
        assert_eq!(
            err_kind("262142-12-31 23:59:60-01"),
            ParseErrorKind::OutOfRange
        );
        assert_eq!(
            parse_timestamptz("262142-12-31 23:59:60+01")
                .unwrap()
                .to_string(),
            "+262142-12-31 23:00:00 UTC",
        );
        // A rollover that stays on the maximum date is fine.
        assert_ok!(parse_timestamp("262142-12-31 23:58:60"));
    }
