...
use mz_storage_types::connections::{
    ...
    KafkaConnection, KafkaSecurity, KafkaTlsConfig, PostgresConnection, SaslConfig, SaslTlsConfig,
...
impl TryFrom<&ConnectionOptionExtracted> for Option<KafkaSecurity> {
    ...
    fn try_from(v: &ConnectionOptionExtracted) -> Result<Self, Self::Error> {
        ...
        let security_protocol = match security_protocol.as_deref() {
            ...
            // specified, we always choose one of the SSL-enabled protocols,
            // using the presence of any SASL options to guide us between them.
            // Users must explicitly choose a plaintext mechanism if that's
            // what they want.
        ...
        let out = match security_protocol {
            ...
            SecurityProtocol::SaslPlaintext | SecurityProtocol::SaslSsl => {
                ...
                let Some(sasl_mechanism) = &v.sasl_mechanisms else {
                ...
                let tls = if matches!(security_protocol, SecurityProtocol::SaslSsl) {
                    outstanding.remove(&ConnectionOptionName::SslCertificateAuthority);
                    Some(SaslTlsConfig {
                        root_cert: v.ssl_certificate_authority.clone(),
                    })
                ...
                Some(KafkaSecurity::Sasl(SaslConfig {
                    ...
                    sasl_mechanism: sasl_mechanism.to_uppercase(),
