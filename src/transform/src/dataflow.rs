...
use mz_expr::{
    ...
    MirScalarExpr, PersistOrIndex, RECURSION_LIMIT,
};
                    ...
                    MirRelationExpr::Get {
                        ...
                        if let Some(new_idx_id) = full_scan_changes.get(global_id) {
                            match persist_or_index {
                                ...
                                Some(PersistOrIndex::Persist) => {
                                    // We already know that it's an indexed access.
                                    unreachable!()
                                }
                                Some(PersistOrIndex::Index(accesses)) => {
            ...
            Ok(match expr {
                ...
                MirRelationExpr::Get {
                    ...
                    if index_accesses.is_empty() {
                        *persist_or_index = Some(PersistOrIndex::Persist);
                    } else {
                        *persist_or_index = Some(PersistOrIndex::Index(index_accesses));
                    }
                }
                MirRelationExpr::Get {
