...
impl Default for RocksDBTuningParameters {
    fn default() -> Self {
        Self {
            ...
            paralleism: defaults::DEFAULT_PARALLELISM,
...
impl RocksDBTuningParameters {
    ...
    pub fn from_parameters(
        ...
        Ok(Self {
            ...
            paralleism: match paralleism {
                Some(paralleism) => {
                    if paralleism < 1 {
                        return Err(anyhow::anyhow!(
                            "paralleism({}) must be > 1, or not specified",
                            universal_compaction_target_ratio
                        ));
                    }
                    Some(paralleism)
    ...
    pub fn apply_to_options(&self, options: &mut rocksdb::Options) {
        ...
        let paralleism = if let Some(paralleism) = paralleism {
            *paralleism
        } else {
        ...
        options.increase_parallelism(paralleism);
    }
}
